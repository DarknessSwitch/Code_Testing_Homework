    ...
    internal class PurchaseService : IPurchaseService
    {
        ...
        public void Purchase(int accountId, IEnumerable<int> productIds, decimal total)
        {
            using (var uow = _unitOfWorkFactory.Create())
            {
                ...
                {
                    // Added exception thrown when product's amount is 0
                    if (product.Amount==0)
                    {
                        throw new PurchaseException(PurchaseError.NotEnoughAmount, string.Format("{0} is out of stock", product.Title));
                    }
                    product.Amount--;
                    account.Balance -= product.Price; // Fixed account balance updating
                }
